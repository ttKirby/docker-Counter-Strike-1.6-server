...
if [[ -f /hlds/cstrike/addons/metamod/dlls/metamod_i386.so ]] && \
  ...
  wget -qO- "http://prdownloads.sourceforge.net/metamod/metamod-1.20-linux.tar.gz" | tar -zxf - -C /hlds/cstrike/addons/metamod/dlls
  ...
  # mkdir -p /hlds/cstrike/addons/metamod/dlls && \
  # wget -qO /tmp/metamod.zip "https://www.amxmodx.org/release/metamod-1.21.1-am.zip" && \
